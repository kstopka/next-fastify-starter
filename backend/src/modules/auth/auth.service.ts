...
import { z } from "zod";
import { findUserByEmail, createSession, findSessionByToken, revokeSession } from "./auth.repository";
import crypto from "node:crypto";

const JWT_SECRET = process.env.JWT_SECRET ?? "change_me";
const ACCESS_EXPIRES_SECONDS = Number(process.env.ACCESS_EXPIRES_SECONDS ?? 60 * 15); // 15 min
...
export const generateAccessToken = (payload: object) => {
  return jwt.sign(payload, JWT_SECRET, { expiresIn: `${ACCESS_EXPIRES_SECONDS}s` });
};
